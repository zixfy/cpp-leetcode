...
//  "cdefab"，
//"efabcd"， 和 "efcdab" 都是串联子串。 "acdbef" 不是串联子串，因为他不是任何
// words 排列的连接。
//
//
...
//
//
//  Related Topics 哈希表 字符串 滑动窗口 👍 951 👎 0
...
public:
  vector<int> findSubstring(string s, vector<string> &words) {
    const auto n = s.length(), m = words.size();
    const auto frac = words.front().size();
    if (n < frac)
      return {};
    unordered_map<string_view, int> word_dict;
    for (string_view w : words)
      word_dict[w]++;
    vector<int> ans;
    auto tmp = word_dict;
    for (int off = 0; off < min(n, frac); ++off) {
      word_dict = tmp;
      for (int l = off, i = off; i <= n - frac; i += frac) {
        // [l, j)
        string_view cur_s(s.c_str() + i, frac);
        if (word_dict.count(cur_s) == 0) {
          l = i + frac;
          word_dict = tmp;
        } else {
          --word_dict[cur_s];
          while (word_dict[cur_s] < 0) {
            ++word_dict[string_view(s.c_str() + l, frac)];
            l += frac;
          }
        }
        if ((i - l) == (m - 1) * frac)
